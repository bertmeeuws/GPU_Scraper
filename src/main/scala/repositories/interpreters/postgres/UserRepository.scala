...
class UserRepositoryInterpreters(xa: Transactor[IO]) extends UserRepository[IO] {
  ...
  override def find(userId: Long): IO[Option[User]] =
    IO.println(s"Finding user with id: $userId") >> sql"SELECT id, username, password FROM public.user WHERE id = $userId"
      .query[User]
      .option
      .transact(xa)

  override def create(user: UserWithOutId): IO[Long] =
    IO.println(s"Creating user with data: ${user.username}") >> sql"INSERT INTO public.user (username, password) VALUES (${user.username}, ${user.password})".update
